...
{#ff0055}颜色{}也接受算术操作，它们是{#aa4444}可加{}、{#aa4444}可乘{}、{#aa4444}可舍入{}、{#aa4444}可平均{}的片段。

;;;;;

能一次性接受多个输入的曲变术会逐输入执行其操作。例如：
...
[常量修订术](^trickster:constants)，会将符记替换为数“2”。可用此修订术和后页各戏法制造出其他数。
...
给定两个数。返回值是在计算幂时，为让底数为第一个数的幂等于第二个数，指数应为的数值。
