...
白名单/黑名单，匹配任一/匹配全部，以及所有条目独有规则在输入/输出端的表现都遵守上方的说明。特别地，库存量在两端的意义与上方相反——“保留一定量”和“限制接收量”和上方所说的并不是一回事。
...
选择**普通过滤器**后可打开它的**45个条目槽**。

- 精确匹配物品ID和流体ID。
- 向条目槽放入物品或流体桶可将其加入过滤列表。
- 从JEI中往槽位拖放物品或流体，即可新增虚影规则。
- 移除物品即是移出列表。
- 忽略空条目槽。

每个条目都可打开详情页面，可在其中设置标签匹配、单次数量覆盖、库存量阈值、NBT规则，以及对所依附容器的槽位限制。

在节点频道中打开此类过滤器时，**扫描所依附容器**按钮可自动用容器和储罐的内容物填充空条目。界面会报告新增条目的数量、过滤器是否被填满、是否没有新增项、以及是否不存在兼容的存储空间。扫描功能适用于物品和流体频道。
...
正则表达式过滤器使用受限、大小写不敏感的正则表达式语法匹配资源的显示名称。
...
普通过滤器主界面的所有槽位都不只是单个物品的匹配项。打开已配置条目的详情页面可配置条目独有的规则：
...
过滤器界面中可以复制和粘贴过滤器的整套设置。使用**Ctrl+C**以复制，**Ctrl+V**以粘贴，或者也可对过滤器界面标题右上角的**复制**和**粘贴**图标按下**主要交互键（默认：左键）**。将鼠标悬停在两按钮上可显示提示文本；在剪贴板中有复制内容粘贴按钮会变暗。

该剪贴板仅会在当前游戏会话中保存。它**不会**被保存到过滤器物品、配置，或是操作系统的剪贴板，且会在离开游戏时清除。它和扳手的[复制/粘贴](../wrench/copy-paste.md)剪贴板相独立。

复制的内容取决于过滤器类型：

- **普通**：所有条目与各条目独有的规则——物品、流体、`#标签`、单次数量和库存量、槽位映射、NBT（严格标记、原始SNBT、规则、匹配任一/全部模式）、耐久度、附魔状态，同时也会复制过滤器的白名单/黑名单模式和类型。
