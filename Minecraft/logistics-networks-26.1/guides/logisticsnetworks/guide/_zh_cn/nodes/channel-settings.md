...
按住**修饰键3（默认：Alt）**再按下**主要交互键（默认：左键）**可设为最大值，或可再按下**次要交互键（默认：右键）**设为最小值。

对界面顶部的频道编号按钮按下主要交互键可切换改动应用于哪一个频道。
...
**如何改动：**&zwnj;对值按钮按下主要交互键可在已启用和已禁用间切换。
...
**如何改动：**&zwnj;对值按钮按下主要交互键可在输出和输入间切换。
...
- **物品**：物品容器（箱子、熔炉、漏斗、AE2接口、精致存储接口等）中的物品堆叠。
- **流体**：储罐中的流体，以毫桶（mB）计。
- **能量**：能量存储空间中的Forge能量/RF。

**如何改动：**&zwnj;对值按钮按下主要交互键可在可用类型间循环切换。

**小提示：**&zwnj;不是所有方块都支持所有类型。若方块在对应面没有相应能力，频道便什么都不会做，且不会发出通知。把物品节点贴在流体储罐上便什么都不会传输，因为储罐没有物品存储空间。

### AE2和精致存储接口

依附于应用能源2接口和精致存储接口的节点会直接对相连的网络输入输出物品和流体。输出端能抽取的资源由接口的配置决定，也接受接口原生的模糊匹配，不过单次数量不受接口配置的暂存量限制。输入端可向网络输入节点频道过滤器指定的所有资源。

这种直接发送行为会忽略接口公开的物品/流体存储空间。显式指定槽位映射会让物品频道切换回接口公开的物品容器。存储网络离线时，节点进入等待状态，不会消耗接口内容物；直接发送不会请求自动合成。

化学品（通用机械）和魔源（新生魔艺）的兼容保留到未来使用，当前无效。
...
**如何改动：**&zwnj;按下主要交互键可循环切换至下一个面（顺序为上 → 下 → 北 → 东 → 南 → 西 → 所有 → 上……）。
...
- **轮询分配**：在匹配的各输入端间尽可能均分每次操作运输的所有物品。例如，送至两处空输入端的一批8个常规物品会被均分为每处4个。

**如何改动：**&zwnj;按下主要交互键以循环切换至下一模式。

**小提示：**&zwnj;轮询分配不会存储轮转指针。此模式会均分单次操作送出的所有可用物品，未成功送出的部分会进入有空闲存储空间的输入端。
...
**它的功能：**&zwnj;供分配设置为**优先级**的输出端使用。输出端会按照此数为目标输入端排序，值大者优先，并按此顺序传输。物品按顺序从来源槽中读取，因此当某批次需从多个来源槽取出时，首个输入端可能会被前几个来源槽填满，这之后才会将剩余资源送至下一个输入端。

**如何改动：**&zwnj;对数字框按下主要交互键以打开文本框，输入在-99和99之间的数，然后按Enter。
...
**如何改动：**&zwnj;对数字框按下主要交互键以打开文本框，输入新值，然后按Enter。最小可为1。
...
**如何改动：**&zwnj;对数字框按下主要交互键以打开文本框，输入新值，然后按Enter。最小可为1刻。
