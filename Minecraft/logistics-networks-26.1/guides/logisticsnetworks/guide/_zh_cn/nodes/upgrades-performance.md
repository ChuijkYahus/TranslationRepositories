...
换用高等级升级时会将各频道的单次数量设为新的最大值。物品和流体频道的延迟不会变动，如需加速请自行修改。能量频道的延迟若锁定则不变。移除高等级升级时会根据移除后的等级限制各值。剪贴板和标签配置均不变。
...
若同时安装了铁级、金级、钻石级升级，节点会选用**钻石级**的上下限。铁级和金级不会被使用——升级本身无法堆叠。节点需要跨维度传输时，应在升级槽中放入[跨维度升级](upgrades-special.md#跨维度升级)；通用机械化学品升级和新生魔艺魔源升级目前暂无作用。
