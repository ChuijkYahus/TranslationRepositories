...
- Only upgrade items are accepted. Regular items are rejected.
...
The Mekanism Chemical and Ars Source upgrades are retained for future compatibility, but they do not currently enable those integrations.

**No duplicates.** You cannot install two of the same upgrade in one node. The slot refuses any upgrade that is already present in the other slots.

**How to install:** use Primary Interaction to place an upgrade item into an empty slot, or use Modifier Key 1 + Primary Interaction from your inventory. Use the same controls to remove it.
