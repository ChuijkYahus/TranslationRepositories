...
![过滤条目的详情页面，内有物品/标签、单次数量、库存量、NBT、槽位](images/advanced-detail-full.png)
...
![库存量行](images/advanced-stock.png)

条目的数量阈值。仅这一个字段的意义在输出端和输入端上**存在不同**：

- **输出端**：库存量的意义是**存量**。若抽取后会导致来源方块中对应资源的数量低于该值，则不进行抽取。示例：将煤炭条目的库存量设为`8`，输出端便会在来源箱子中保留8个煤炭。
- **输入端**：库存量的意义是**上限**。目的地中资源已达到库存量时，不再接收。示例：将铁锭条目的库存量设为`64`，输入端便会在目的地已有64个铁锭时停止补货。
