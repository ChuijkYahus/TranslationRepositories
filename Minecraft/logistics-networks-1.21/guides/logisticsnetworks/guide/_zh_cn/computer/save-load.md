...
2. 在所挂载网络面板中，按下**保存网络**（`写入.lnet文件`）。
3. 电脑会立即导出挂载的网络，并在面板中反馈结果。

电脑会将其写入`<网络名称>.lnet`。再次保存同一个网络会**覆写**已有文件。

### 每个节点都需要标签

只有在**网络内所有节点都有标签**时，才可进行导出。标签是文件将已保存的配置加载回节点的判定标记。标签若空缺即会阻止导出，不会产生文件，且会报告错误：

- `缺失标签：X个节点`：部分节点没有标签。需要为它们设置标签。

标签重复不会阻止导出。导出时会保留遇到的首个标签条目，并跳过后续的同标签条目。
...
5. 取走扳手。通过**修饰键1+次要交互键**（默认：**Shift+右键**）将配置复制到节点中。粘贴操作参见[复制与粘贴](../wrench/copy-paste.md)。
