...
对于物品输出端而言，直接发送时会首先取存储网络中的资源，之后才会取接口中暂存的资源。于是，后续的传输可使用缓存物品，从而排空接口。流体直接发送仍会忽略公开的流体容器。显式指定槽位映射会让物品频道切换回接口公开的物品容器。存储网络离线时，节点进入等待状态，不会消耗接口内容物；直接发送不会请求自动合成。
...
- **轮询分配**：在匹配的各输入端间尽可能均分每次操作运输的所有物品。
